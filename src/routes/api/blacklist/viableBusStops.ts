...
type TmpDatabase = Database & { 'busstopzone': CoordinatesTable } & { 'busstoptimes': TimesTable };
...
const withBusStops = (busStops: BusStop[], startFixed: boolean) => {
  return db
    .with('busstops', (db) => {
      ...
      return db
        .selectFrom(
          sql<CoordinatesTable>`(${sql.join(busStopsSelect, sql<string>` UNION ALL `)})`.as('busstops')
        )
        .selectAll();
    })
    .with('times', (db) => {
      let busStopIntervals: RawBuilder<string>[] = busStops.flatMap((busStop, i) => busStop.times.map((t, j) => {
      ...
      return db
        .selectFrom(sql<TimesTable>`(${sql.join(busStopIntervals, sql<string>` UNION ALL `)})`.as('times'))
        .selectAll();
    });
};

const doesAvailabilityExist = (eb: ExpressionBuilder<TmpDatabase, 'vehicle' | 'busstoptimes'>) => {
  return eb.exists(
    eb
      .selectFrom('availability')
      .whereRef('availability.vehicle', '=', 'vehicle.id')
      .whereRef('availability.startTime', '<=', 'busstoptimes.endTime')
      .whereRef('availability.endTime', '>=', 'busstoptimes.startTime')
  );
};

const doesTourExist = (eb: ExpressionBuilder<TmpDatabase, 'vehicle' | 'busstoptimes'>) => {
  return eb.exists(
    eb
      .selectFrom('tour')
      ...
      .where((eb) =>
        eb.and([
          sql<boolean>`tour.departure <= busstoptimes.end_time`,
          sql<boolean>`tour.arrival >= busstoptimes.start_time`
        ])
      )
  );
};

const doesVehicleExist = (eb: ExpressionBuilder<TmpDatabase, 'company' | 'zone' | 'busstopzone' | 'busstoptimes'>, capacities: Capacities) => {
  return eb.exists((eb) =>
    eb
      .selectFrom('vehicle')
...
const doesCompanyExist = (eb: ExpressionBuilder<TmpDatabase, 'zone' | 'busstopzone' | 'busstoptimes'>, capacities: Capacities) => {
  return eb.exists(
    eb
      .selectFrom('company')
      .where((eb) =>
        eb.and([
          ...
          doesVehicleExist(eb, capacities)
        ])
      )
  );
};
...
export const getViableBusStops = async (
    ...
    return withBusStops(busStops, startFixed)
      ...
      .innerJoinLateral(
        (eb) =>
          eb
            .selectFrom('times')
            .whereRef('times.busStopIndex', '=', 'busstopzone.busStopIndex')
            .selectAll()
            .as('busstoptimes'),
        (join) => join.onTrue()
      )
      .where((eb) => doesCompanyExist(eb, capacities))
      .select(['busstoptimes.timeIndex as timeIndex', 'busstoptimes.busStopIndex as busStopIndex'])
      .execute();
  };
  ...
  return batchResponses.flatMap((batchResponse, idx) => batchResponse.map((r) => {
    console.log(r);
