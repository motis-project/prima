<script lang="ts">
	...
	import { posToLocation, type Location as ApiLocation } from '$lib/map/Location';
		...
		{#await fps then f}
			{#if f}
				<Control position="bottom-right">
					<Card class="h-[500px] w-[500px] overflow-y-auto rounded-lg bg-background">
						...
						<TableCell>
							{#if key === 'osm_node_id'}
								<a
									...
									class="bold text-blue-400 underline"
									target="_blank"
								>
									{value}
								</a>
